...
<h1 class="h4 mb-3">New share transfer</h1>
...
<form id="transferForm" method="post" action="<?= url('/transfers') ?>">
  ...
  <div class="row g-3">
    ...
    <div class="col-md-4">
      <label class="form-label">Deed reference</label>
      <input name="deed_reference" class="form-control" placeholder="ACT-… (generated if empty)">
    </div>
  </div>
  <div id="holdingHint" class="form-text mb-2" aria-live="polite"></div>
  <div class="alert alert-light border mt-3 mb-2 small">
    The transfer is recorded in the share movement register once the seller's available shares are validated.
  </div>
  <button class="btn btn-primary">Save transfer</button>
  <a href="<?= url('/transfers') ?>" class="btn btn-outline-secondary">Cancel</a>
</form>
</div></div>
