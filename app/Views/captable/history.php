...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
  <div>
    <h1 class="h4 mb-0"><?= __('Capital history') ?></h1>
    <div class="text-muted small"><?= __('Reconstruction as of') ?> <?= e(date('Y-m-d', strtotime($asOf))) ?> <?= __('from the movement register (art. 716)') ?></div>
  </div>
...
<form class="row g-2 align-items-end mb-3" method="get" action="<?= url('/captable/history') ?>">
  <div class="col-auto">
    <label class="form-label small mb-0" for="as_of"><?= __('As-of date') ?></label>
    <input type="date" id="as_of" name="as_of" class="form-control" value="<?= e($asOf) ?>" max="<?= date('Y-m-d') ?>">
  </div>
  <div class="col-auto"><button class="btn btn-primary"><?= __('Rebuild') ?></button></div>
  <div class="col-auto text-muted small align-self-center">
    <?= shares($totalShares) ?> <?= __('shares') ?> · <?= money($totalCapital) ?>
    <?php if ($totalShares !== $currentShares): ?>
      (<?= __('today') ?>: <?= shares($currentShares) ?>)
    <?php endif; ?>
  </div>
</form>
  ...
  <div class="col-lg-7">
    <table class="table table-hover bg-white shadow-sm">
      ...
      <tbody>
        ...
        <?php if ($holdings === []): ?><tr><td colspan="5" class="text-center text-muted py-4"><?= __('No shares as of this date.') ?></td></tr><?php endif; ?>
      </tbody>
      <tfoot class="table-light fw-bold"><tr><td colspan="2"><?= __('Total') ?></td><td class="text-end"><?= shares($totalShares) ?></td><td class="text-end"><?= money($totalCapital) ?></td><td class="text-end">100 %</td></tr></tfoot>
    </table>
  </div>
  <div class="col-lg-5">
    <div class="card bg-white shadow-sm">
      <div class="card-header"><i class="bi bi-clock-history me-2"></i><?= __('Movements up to this date') ?></div>
      <ul class="list-group list-group-flush" style="max-height: 30rem; overflow-y: auto;">
        <?php foreach ($movements as $m):
          $labels = ['issuance' => __('Issuance'), 'transfer_out' => __('Transfer'), 'transfer_in' => __('Transfer (in)')]; ?>
        ...
        <?php endforeach; ?>
        <?php if ($movements === []): ?><li class="list-group-item text-muted small"><?= __('No movements.') ?></li><?php endif; ?>
      </ul>
    </div>
  </div>
</div>
