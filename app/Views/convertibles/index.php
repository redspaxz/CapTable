...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h1 class="h4 mb-0">Instruments convertibles</h1>
    <div class="text-muted small">OCA (obligations convertibles), BSA (bons de souscription), SAFE — modélisation de la dilution à la conversion</div>
  </div>
...
<div class="row g-3">
  <section class="col-lg-7">
    <div class="card bg-white shadow-sm">
      <div class="card-header">Instruments en circulation</div>
      <table class="table table-sm table-hover mb-0" data-enhance="table">
        <thead><tr><th>Type</th><th>Porteur</th><th class="text-end">Principal</th><th class="text-end">Remise</th><th class="text-end">Cap</th><th class="text-end">Actions modélisées</th></tr></thead>
        <tbody>
          ...
          <?php endforeach; ?>
          <?php if ($instruments === []): ?><tr><td colspan="6" class="text-center text-muted py-4">Aucun instrument convertible.</td></tr><?php endif; ?>
        </tbody>
      </table>
    </div>

    <?php if (in_array(Auth::role(), ['admin', 'finance'], true)): ?>
    <div class="card bg-white shadow-sm mt-3"><div class="card-header">Nouvel instrument</div>
      <div class="card-body">
        <form method="post" action="<?= url('/convertibles') ?>">
          ...
          <div class="row g-2">
            ...
            <div class="col-md-4"><label class="form-label">Porteur *</label>
            ...
            <div class="col-md-3"><label class="form-label">Remise %</label>
              <input type="number" min="0" max="99" step="0.5" name="discount_pct" class="form-control" value="0"></div>
            <div class="col-md-4"><label class="form-label">Cap de valorisation (XAF)</label>
              <input type="number" min="0" name="valuation_cap" class="form-control" placeholder="Optionnel"></div>
          </div>
  ...
  <section class="col-lg-5">
    <div class="card bg-white shadow-sm h-100">
      <div class="card-header">Cap table pro-forma (conversion totale)</div>
      <table class="table table-sm mb-0">
        <thead><tr><th>Porteur</th><th class="text-end">Actuel</th><th class="text-end">Dilué</th><th class="text-end">%</th></tr></thead>
        <tbody>
      ...
      <div class="card-footer text-muted small">Prix de conversion modélisé : cap / titres actuels, minoré de la remise. Simulation indicative — la conversion réelle suit une AGE d'augmentation de capital.</div>
    </div>
  </section>
</div>
